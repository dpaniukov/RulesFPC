...
for subj in subjects:
    ...
    for run in xrange(int(len(ss_data)/32)):
        ...
        for evar in ev1:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev1_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()

        for evar in ev2:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev2_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()

        for evar in ev3:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev3_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()

        for evar in ev4:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev4_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()

        for evar in ev5:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev5_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()

        for evar in ev6:
            data = str(evar)+'\n'
            data_file=open(write_path+"Subject"+subj+"_univar1a_LearnApply_ev6_run"+str(run+1)+".txt",'a')
            data_file.write(data)
            data_file.close()
